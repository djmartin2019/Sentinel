...
import { getTargetsWithStatus } from '@/lib/queries';
...
export default async function TargetsPage() {
  ...
  return (
    <>
      <Header title="Targets" subtitle={`${targets.length} monitored endpoints`} />
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    {['Name', 'Status', 'URL', 'Latency', 'Uptime', 'Interval', 'Last Check', ''].map((h) => (
                      <th
                        key={h}
                  ...
                  {targets.map((target, i) => (
                    ...
                    >
                      <td className="px-4 py-3">
                        <span className="font-mono text-xs font-medium text-text-primary">{target.name}</span>
                      </td>
                      <td className="px-4 py-3">
                        <StatusPill status={target.status} pulse size="sm" />
                      </td>
                      <td className="px-4 py-3 max-w-[200px]">
                        <span className="font-mono text-[11px] text-text-muted truncate block">{target.url}</span>
                      </td>
                      <td className="px-4 py-3 font-mono text-xs">
                        <span className={target.status === 'DOWN' ? 'text-text-muted' : getLatencyColor(target.latencyMs)}>
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-text-muted">{target.interval}s</td>
                      <td className="px-4 py-3 font-mono text-xs text-text-muted whitespace-nowrap">
                        {formatRelativeTime(target.lastChecked)}
                      </td>
                      <td className="px-4 py-3">
                        <Link href={`/target/${target.id}`} className="opacity-0 group-hover:opacity-100 transition-opacity text-text-muted hover:text-accent-cyan">
